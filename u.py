...
def split_str(input_str):
    ...
    for chr in check_operands:
        if input_str.find(
                chr)>-1:  # the find() function returns the index where a sub-string is found. it returns '-1' otherwise
            op = chr  # set op to the operator character/sysmbol that was found

    partitions = input_str.partition(
        op)  # 'partition()' returns a 3-tuple. the first partition [0], the seperator[1], and the last partition[2]
